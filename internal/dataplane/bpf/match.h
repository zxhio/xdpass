...
static __always_inline void apply_index_mask(struct mask_t *candidates, void *map, const void *key,
					     const struct mask_t *wildcard)
{
	struct mask_t *indexed = bpf_map_lookup_elem(map, key);
	if (indexed) {
#pragma unroll
		for (int i = 0; i < RULE_GROUPS; i++)
			candidates->bits[i] &= indexed->bits[i] | wildcard->bits[i];
	} else {
		mask_and(candidates, wildcard);
	}
}
						  ...
						  __u32 pkt_conds)
{
	if (!(pkt_conds & COND_PROTO_TCP))
		mask_and(candidates, &cfg->condition_wildcard_rules[0]);
	if (!(pkt_conds & COND_PROTO_UDP))
		mask_and(candidates, &cfg->condition_wildcard_rules[1]);
	if (!(pkt_conds & COND_PROTO_ICMP))
		mask_and(candidates, &cfg->condition_wildcard_rules[2]);
	if (!(pkt_conds & COND_PROTO_ARP))
		mask_and(candidates, &cfg->condition_wildcard_rules[3]);
	if (!(pkt_conds & COND_TCP_SYN))
		mask_and(candidates, &cfg->condition_wildcard_rules[9]);
	if (!(pkt_conds & COND_TCP_ACK))
		mask_and(candidates, &cfg->condition_wildcard_rules[10]);
	if (!(pkt_conds & COND_TCP_RST))
		mask_and(candidates, &cfg->condition_wildcard_rules[11]);
	if (!(pkt_conds & COND_TCP_FIN))
		mask_and(candidates, &cfg->condition_wildcard_rules[12]);
	if (!(pkt_conds & COND_TCP_PSH))
		mask_and(candidates, &cfg->condition_wildcard_rules[13]);
	if (!(pkt_conds & COND_ICMP_ECHO_REQUEST))
		mask_and(candidates, &cfg->condition_wildcard_rules[14]);
	if (!(pkt_conds & COND_ARP_REQUEST))
		mask_and(candidates, &cfg->condition_wildcard_rules[15]);
}
...
static __always_inline struct rule_meta *match_rule(struct packet_ctx *pkt, __u32 *matched_slot)
{
	...
	apply_index_mask(&candidates, &vlan_index_map, &pkt->vlan_id, &cfg->vlan_wildcard_rules);
	apply_index_mask(&candidates, &src_port_index_map, &pkt->sport, &cfg->src_port_wildcard_rules);
	apply_index_mask(&candidates, &dst_port_index_map, &pkt->dport, &cfg->dst_port_wildcard_rules);
	...
	if (pkt->pkt_conds & (COND_PROTO_TCP | COND_PROTO_UDP | COND_PROTO_ICMP | COND_PROTO_ARP)) {
		...
		apply_index_mask(&candidates, &src_prefix_lpm_map, &src_key, &cfg->src_prefix_wildcard_rules);
		apply_index_mask(&candidates, &dst_prefix_lpm_map, &dst_key, &cfg->dst_prefix_wildcard_rules);
	} else {
